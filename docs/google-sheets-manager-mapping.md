...
Менеджер видит всю производственную строку. В текущей общей internal-role модели менеджер,
сценарист и монтажёр могут изменять доступные рабочие поля независимо от исходной роли автора;
системные идентификаторы, summaries и даты решений остаются read-only.
...
Колонка approval-комментария хранит `scenario_approvals.comment`. Она не дублируется в
`scenario_comments`; история обсуждения ведётся отдельно через comments API.
