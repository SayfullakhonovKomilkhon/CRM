...
API sheet-проекция содержит ровно 24 перечисленных ниже колонки в порядке разделов. Внутренние
...
Если клиент отправляет финальный монтаж на доработку, запрос сначала проверяет менеджер. Клиент
видит read-only решение `final_revision_gate.decision` и ответ менеджера
`final_revision_gate.manager_comment`. При принятии запроса задача возвращается монтажёру; при
отклонении клиентское финальное согласование открывается повторно.
...
| Статус публициста | `publications.publisher_status` | чтение |
| Ссылка DZEN | `publications.dzen_url` | чтение |
| Ссылка YouTube | `publications.youtube_url` | чтение |
| Ссылка TikTok | `publications.tiktok_url` | чтение |
| Ссылка Instagram | `publications.instagram_url` | чтение |
| Опубликовано в | `publications.published_at` | чтение |

Описания готовят внутренние роли, менеджер подтверждает публикацию и назначает публициста,
а платформенные ссылки и факт публикации устанавливает назначенный публицист. Для клиента все
эти поля read-only.
