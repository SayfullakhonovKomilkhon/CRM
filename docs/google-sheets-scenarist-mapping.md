...
Статус: утверждённая двусторонняя интеграция. Google передаёт только явно
отправленные поля сценариста; CRM записывает обратно настроенные поля всех ролей.

## Идентификация источника

Данные источника отслеживаются следующими полями:

- `source_sheet_id` — ID Google Spreadsheet;
- `source_tab` — название вкладки/проекта;
- `source_row` — номер строки для диагностики;
- `source_checksum` определяет повторные неизменившиеся отправки.

Видимый `external_id` импортируется из колонки `ID` и сохраняется без
перенумерации. Это бизнес-номер строки: он может повторяться в разных исходных
таблицах и остаётся read-only в веб-интерфейсе.
За техническую связь отвечает отдельный защищённый UUID `crm_row_id`.

## Основной сценарий

CRM не импортирует рабочие черновики автоматически. Строка становится доступна
менеджеру сценаристов только после выбора `Отправить` в колонке
`Отправка на согласование`. Все остальные строки, включая тестовые и частично
заполненные черновики, остаются только в Google Sheets. После успешного приёма
отметка очищается; для повторной отправки после доработки сценарист снова
выбирает `Отправить`.
...
- колонки сценариста (основное, исследование, сценарий, подготовка исходника и
  подготовка публикации) работают `Google Sheets ↔ CRM`;
- Google → CRM выполняется только после значения `Отправить`;
- согласования клиента и менеджеров, назначения, результат монтажёра и результат
  публициста работают только `CRM → Google Sheets`;
- изменение CRM-owned ячейки напрямую в Google Sheets игнорируется backend и не
  может изменить workflow.

Утверждённый шаблон сохраняет исходные колонки `A:BN`, `BZ` использует как
защищённый `crm_row_id`, а новые CRM-only колонки размещает в свободных
диапазонах `BO:BY` и `CA:CS`. Заголовки новых колонок backend добавляет при
первой записи соответствующей полной строки; существующие колонки не
