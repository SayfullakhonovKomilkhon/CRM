...
`external_id` не импортируется из Google Sheets. CRM назначает его автоматически как глобальный
последовательный номер по порядку создания строки. Поле read-only, номера не переиспользуются.
