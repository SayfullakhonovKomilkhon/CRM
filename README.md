...
- Scenario `external_id` is read-only in the web CRM. Web-created scenarios
  receive a PostgreSQL sequence value; Sheet-created scenarios preserve the
  globally unique visible Google `ID` and advance that sequence when numeric.
...
For a Sheet-created scenario, the visible Google `ID` is preserved as the CRM
`external_id`; the protected `crm_row_id` UUID is used only to bind the row.
